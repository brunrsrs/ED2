...
do {
    ...
    switch(op) {
        ...
        //Operações no turma.dat
        //Tenta abrir o arquivo pra leitura e escrita
        arqDados = fopen("turma.dat", "r+");
        if (!arqDados)
            arqDados = fopen("turma.dat", "w+");

        //Operações no index.dat
        //Tenta abrir o arquivo pra leitura e escrita
        ...
        fprintf(arqDados, "%02ld%s|%02ld%s|%d|", strlen(A.nome), A.nome, strlen(A.curso), A.curso, A.RA_UNESP);
            //o 2 são os numeros de chars de cada .dat, o 9 é o N° do RA que tem 9 digitos e 3 são as "|"
        ...
        printf("\nAluno: %s\nCurso: %s\nRA: %d\n\n", A.nome, A.curso, A.RA_UNESP); //Mostra na tela o resultado
        break;
